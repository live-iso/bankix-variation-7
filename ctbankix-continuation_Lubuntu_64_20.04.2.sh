...
#	Copyright (C) 2021 ctbankix-continuation-team
#
...
echo
echo "Bauanleitung - Lubuntu 20.04.2 64 Bit"
echo "====================================="
echo
...
apt-get -y build-dep linux-image-unsigned-$(uname -r)
...
sed -i '/^CONFIG_BLK_DEV_NVME/d' debian.hwe-5.8/config/config.common.ubuntu
#sed -i '/^(# )?CONFIG_NVME_/d' debian.hwe-5.8/config/config.common.ubuntu

sed -i '/^CONFIG_NVME_/d' debian.hwe-5.8/config/config.common.ubuntu
sed -i '/^# CONFIG_NVME_/d' debian.hwe-5.8/config/config.common.ubuntu


cat >> debian.hwe-5.8//config/config.common.ubuntu << EOF
...
sed -i "/^CONFIG_BLK_DEV_NVME/ s/'.'/'n'/g" debian.hwe-5.8/config/annotations
sed -i "/^CONFIG_NVME_/ s/'.'/'n'/g" debian.hwe-5.8/config/annotations
sed -i "/^CONFIG_NVME_TARGET_TCP/ s/'.'/'-'/g" debian.hwe-5.8/config/annotations
sed -i "/^CONFIG_NVME_TARGET_FCLOOP/ s/'.'/'-'/g" debian.hwe-5.8/config/annotations
sed -i "/^CONFIG_NVME_MULTIPATH/ s/'.'/'-'/g" debian.hwe-5.8/config/annotations
sed -i "/^CONFIG_NVME_HWMON/ s/'.'/'-'/g" debian.hwe-5.8/config/annotations
sed -i "/^CONFIG_NVME_TARGET_FC/ s/'.'/'-'/g" debian.hwe-5.8/config/annotations
sed -i "/^CONFIG_NVME_TARGET_LOOP/ s/'.'/'-'/g" debian.hwe-5.8/config/annotations
sed -i "/^CONFIG_NVME_TARGET_RDMA/ s/'.'/'-'/g" debian.hwe-5.8/config/annotations
sed -i "/^CONFIG_NVME_TCP/ s/'.'/'-'/g" debian.hwe-5.8/config/annotations
...
wget -c -N -P source http://cdimage.ubuntu.com/lubuntu/releases/20.04.2/release/lubuntu-20.04.2-desktop-amd64.iso
